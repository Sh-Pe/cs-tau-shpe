...
\usepackage{fancyhdr}
\pagestyle{fancy}
\fancyhead[C]{\textit{\textbf{אלגברה לינארית 2א}}}
\fancyhead[L,R]{}
...
\date{2025 סמסטר ב'}
\begin{document}
	\renewcommand{\footrule}{\rule{\linewidth-18pt}{0.25pt}\vspace{-5pt}}
	\thispagestyle{empty}
	\,\!
...
%		\end{proof}
	...
	\renewcommand{\footrule}{\rule{\linewidth-25pt}{0.25pt}\vspace{-5pt}}
	\npage
...
%	למעשה, $d$ הנ''ל הוא המינימלי שעבורו ניתן לבטא את $A^{d}$ כצ''ל של חזקות נמוכות יותר. 
	...
	\defi{יהי $V$ מ''ו, \textit{מכפלה פנימית} מעל $\R$ היא תבנית בילינ' סימטרית חיובית מעל $V$, ומסומנת $f(v, u) = \la v, u \ra$ (ויש ספרים שמסמנים $\la v \mid u \ra$, $\smut\co V \times V \to \R$. }
...
%		TODO: to make it clear
	...
	\begin{itemize}
		...
		\item נחשב את $V_{\lg_j}^{(i)} := \nc((A - \lg_j)^{\ell_j})$ עד שנקבל $\dim\cl{V_{\lg_j}^{(\ml_j)}} = r_i$ (המרחב העצמי המוכלל). 
		\item נחזור על האלגו' למציאת צורת ג'ורדן למטריצה ניל': 
		\begin{itemize}
