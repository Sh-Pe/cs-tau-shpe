...
%	למעשה, $d$ הנ''ל הוא המינימלי שעבורו ניתן לבטא את $A^{d}$ כצ''ל של חזקות נמוכות יותר. 
	...
	%TODO: don't do \Sg \co M_{r \times r}, but an M_{m \times n} diagonal matrix. It makes more sense. 
...
%		נגדיר $T_A(x) = Ax, \, T_A \co \R^{n} \to \R^{m}$. נבצע פירוק באופן הבא: נגדיר $W = \ker A$ עם בסיס כלשהו $B_W$, ותהי הרחבה לבסיס $B$ כאשר $B_E := B \setminus B_W$ ו־$E = \Sp B_V$. קל להראות ש־$\dim E = \rk A$ וכן $A(E) = \col A$. ברור ש־$(T_A)|_{E}$ איזו' מקסימלי ו־$\Img (T_A)|_{E} = \Img T_E = \col A$. נקבל שהמטריצה המייצגת של $(T_A)|_E$ בבסיס הסטנדרטי $\ec$ (נסמנה $A_E$) היא תת־מטריצה של $A$. משום ש־$A_E$ הפיכה קיים לה פירוק SVD $A_V = \tl U \Sg \tl V^T$ יחיד, כאשר $U, V \in M_{r \times r}$. נרחיב את עמודות $U, V$ לבסיס אורתונורמלי של $\R^{n}, \R^{m}$ בהתאמה, ומהשורות המתקבלות נקבע מטריצות $U, V$ חדשות (תוך שמירה על הסדר). עתהנראה ש־$A = U\Sg V^T$. יהי $x \in \R^{n}$, ותרגיל נפוץ בלינארית אחת להראות ש־$W = \ker A$ מקיים $E\oplus W = \F^{n}$, ואז נוכל לכתוב את $x$ כסכום $x = e + w, e \in E, w \in W$ כלשהו. בגלל ש־$w \in \ker A$, נקבל $Ax = Ae + Aw = Ae$ וקל להראות שבגלל ש־$A_V = \tl U \Sg \tl V^T$ אז $Av = U\Sg V^T$. סה''כ הוכחנו את הדרוש. 
	...
	\begin{proof}נסמן $\dim V = n, \dim W = m$
		...
		\[ [T]_\cc^\bc = V \Sg U^T \]
		כאשר $V, U$ אוניטריות ו־$\Sg$ אלכסונית. ממשפט ידוע שעל אלכסון $\Sg$ מופיעים $\sg_1 \dots \sg_n$. בגלל ש־$V$ מטריצה עם $r$ שורות ב־$\R^{m}$ נסמנן $V_1 \dots V_r$ ובאופן דומה $W$ מטריצה עם שורות $W_1 \dots W_r$ ב־$\R^{n}$, נוכל להגדיר (כאשר $[]_B\op$ ההעתקה ההופכית לייצוג בבסיס $B$)
		\[ \urm_i = [U_i]_\bc\op \quad \vrm_i = [V_i]_\cc\op \]
		עתה נשאר להראות שהבחירה שלנו אכן עובדת. יהי $v \in V$, ונסמן $[v]_\bc = (a_1 \dots a_n)$. 
		...
		\[ \begin{WithArrows}
			...
			= V \Sg \sum_{i = 1}^{r}a_1 U_i \Arrow[ll]{ייצוג של $\,\!_iU$ ב־$\,\!^{r}\R$ בבסיס $\ec$} \\
