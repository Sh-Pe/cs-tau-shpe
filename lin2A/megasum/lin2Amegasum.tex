...
\newcommand\prm   {\bm{\mathrm{p}}}
\newcommand\arm   {\bm{\mathrm{a}}} % x86
\newcommand\brm   {\bm{\mathrm{b}}}
\newcommand\crm   {\bm{\mathrm{c}}}
\newcommand\drm   {\bm{\mathrm{d}}}
\newcommand\erm   {\bm{\mathrm{e}}}
\newcommand\frm   {\bm{\mathrm{f}}}
\newcommand\nrm   {\bm{\mathrm{n}}}
\newcommand\vrm   {\bm{\mathrm{v}}}
\newcommand\urm   {\bm{\mathrm{u}}}
\newcommand\srm   {\bm{\mathrm{s}}}
\newcommand\rrm   {\bm{\mathrm{r}}}
...
%	\AddToShipoutPictureBG*{\includegraphics[width=\paperwidth,height=\paperwidth]{background.png}}
...
%	\color{red}\textbf{אזהרה!}\color{black}\, נכון למצב הנוכחי, הסיכום הזה עדיין בעבודה לצריך לעבור הגהה. אתם מוזמנים להשתמש בו, אך אין לראות בו כרגע גרסה סופית ואמינה. 
	...
	\subsection{רשימת נושאים שנוספו לסיכום בהתווסף על החומר של הקורס}
	\begin{itemize}
		\item מציאת צורת ג'ורדן באמצעות מרחבים עצמיים מורחבים (עוזר מאוד להבין מה צורת ג'ורדן עושה). 
		\item תוצאות מצורת ג'ורדן (מופיע בסמסטרים קודמים). 
		...
		\item מרחבים דואלים (המרצה עשה על זה הרחבה קצרה בסיום ההרצאה האחרונה)
	\end{itemize}
...
%		\end{proof}
	...
	\theo{תהי $T \co V \to V$ ט"ל עם פ"א $f_T(x)$. אז $T$ לכסינה אמ"מ שתי הטענות הבאות מתקיימות:
		\begin{enumerate}
			\item בעבור $k$ הע"ע שונים, $f_T(x) = \prod_{i = 1}^{k}(x - \lg_i)^{d_\lg}$
	...
	\rmark{באנגלית, Principal Ideal Domain או בקיצור PID. }
...
%	\end{proof}
	...
	\begin{Theorem}[משפט ההתאמה של סילבסטר]
		$p, q$ עבורם המטריצה חופפת ל־$\diag(I_p, I_{-q}, 0)$ נקבעים ביחידות. 
	\end{Theorem}
	...
	\[ (\cos \widehat{u, v}) := \frac{\mut{u}{v}}{\norm{u}\norm{v}} \quad\quad \widehat{u, v} = \arccos(\cos(\widehat{u, v})) \]}
	...
	\theo{נניח $V$ ממ''פ מעל $\C$. אז $T$ הרמיטית אמ''מ $\forall v \in V \co \mut{Tv}{v} \in \R$}
...
%	\theo{העתקה סימטרית אמ''מ היא דומה למטריצה סימטרית. }
	...
	\defi{$A \in M_n(\F)$ תקרא חיובית/אי־שלילית/שלילית/אי־חיובית אם לכל $v \in \F^{n}$: 
		\begin{multicols}{2}
			חיובית: $\mut{Av}{v} > 0$ \\
			שלילית: $\mut{Av}{v} < 0$ \\
			אי־שלילית: $\mut{Av}{v} \ge 0$ \\
			אי־חיובית: $\mut{Av}{v} \le 0$
		\end{multicols}
		כאשר $\smut$ המ''פ הסטנדטית מעל $\F^{n}$. 
	}
	\rmark{זהו אינה הגדרה נפוצה. לרוב מדברים רק על מטריצה מוגדרת חיובית. }
	...
	\begin{proof}
		...
		\begin{enumerate}
			...
			\item[$3 \to 1$] ידוע קיום $\ec$ כך ש־$[T]_\ec = [T]_\ec^* = [T^*]_\ec$. לכן מהיות $[\cdots]_\ec$ העתקת הקורדינאטות לבסיס איזומורפיזם, בהכרח $T = T^*$ כדרוש. 
		\end{enumerate}\envendproof
	\end{proof}
	...
	\end{proof}
	הוכחה שהמשפט מתקיים בהכרח מעל המרוכבים: ההעתקה $T(x) = ix$ היא העתקה סקלרית לינארית, לכן כל וקטור הוא ו''ע וכל בסיס מלכסן, בסיס אורתונורמלי כלשהו יהיה בסיס מלכסן על אף שההעתקה לא צמודה לעצמה, אלא אנטי־הרמיטית. 
	...
	\end{proof}
	\rmark{בניסוחים אחרים, מדברים על $A$ הרמיטית, במקום על $f$ סימטרית. שימו לב שבכל מקרה אין משמעות למשפט מעל המרוכבים (שכן במקרה הזה $\sg_- = 0 \land \sg_+ = \rk f$, וכן כל מספר מרוכב נוכל לנרמל ל־$1$) ולכן שני הניסוחים חזקים באותה המידה. }
	
	\cola{מכאן, שבהינתן $A$ מטריצה הרמיטית חיובית, היא מייצגת תבנית בי־לינארית חיובית וגם מייצגת העתקה חיובית. למעשה, אפשר להוכיח שבהינתן $A$ הרמיטית, היא חיובית (בהבט של המכפלה הפנימית) אמ''מ היא חיובית (בהבט של תבניות בי־לינאריות). }
	...
	\theo{בפירוק פולארי $T = RU$, כאשר $R$ הרמיטית חיובית, אז בהכרח $R = \sqrt{TT^*}$ ולכן יחידה. }
	\begin{proof}
		\[ TT^* = RU(RU)^* = RUU^*R^* = R^2 \]
	...
	\rmark{אז איך זה קשור לפולארי? $R$ האי־שלילית היא ''הגודל``, בעוד $U$ האוניטרית לא משנה גודל – היא ה''זווית''. ניתן לראות זאת גם באופן הבא: בהינתן $A = RU$ פירוק פולארי ל־$U$ אורתוגונלית ו־$R$ מוגדרת חיובית הרמיטית, אז $\det A = \sof{\det R} =: r$ וגם $\sof{\det U} = 1$ כלומר $\det U = e^{i\ta}$, ואז מקבלים $\det A = \det U \det R = re^{i\ta}$. }
...
%	\end{proof}
	...
	\begin{proof}
		בויקיפדיה האנגלית
	\end{proof}
	\theo{פירוק SVD יחיד אמ''מ הערכים הסינגולרים שונים. }
...
%	\end{proof}
	...
	\rmark{גם תת־הפרק להלן לא בחומר הרשמי של הקורס. אבל חשבתי שזה מגניב אז הוספתי את זה. }
	\defi{ה\textit{נורמה} של העתקה $T \co V \to W$ מממ''פים מוגדרת להיות: 
	\[ \norm{T} = \max\{\norm{Tv} \co v \in V \land \norm{v} \le 1\} \]
	}
	\rmark{אינטואציה גיאומטרית טובה היא לחשוב על $\norm{T}$ כעל ''הכדור המינימלי שחוסם את $Tu$`` כאשר $u$ נורמלי. }
	...
	\begin{enumerate}[A']
		...
		\item \textbf{מציאת פירוק SVD: }בהינתן $T$ העתקה, נמצא את הפירוק הספקטרלי של $TT^*, T^*T$ ומהזהויות		 $TT^* = U\Sg^2U^{T}$ ו־$T^*T = V\Sg^2 V^{T}$, נקבל $T = U\Sg V^{T}$. 
	\end{enumerate}
	...
	\exec{תהי $T$ מעל ממ''פ מרוכב, ו־$f_T = g \cdot h$ כאשר $g, h \in \F[x]$ פולינומים זרים, 
		\begin{enumerate}[A']
	...
	\exec{הוכח שלכל $T$ נורמאלית, עם ע''עים $\lg_1 \dots \lg_k$, מתקיים לכל $n, m \in [k]$ ש־$\forall v \in V_{\lg_n}, \tl u \in V_{\lg_m} \co \mut{v}{u} = 0$ (דהיינו $V_{\lg_n}$ ניצב ל־$V_{\lg_m}$). }
	\exec{הוכח שלכל $T \co V \to V$ מתקיים $\ker(T^*) = \Img(T)^{\perp}$ וגם $\Img(T^*) = \ker(T)^{\perp}$}
	\exec{הוכח שלכל $T \co V \to V$ נורמלית, מתקיים: 
		\begin{enumerate}[A']
			\item \hfil $\ker T = \ker T^*$
			\item \hfil $\Img T = \Img T^*$
			\item \hfil $V = \ker T \oplus \Img T$
		\end{enumerate}}
	\exec{ללא שימוש בפירוק SVD, הראה שהערך הסינגולרי הגדול ביותר והקטן ביותר חוסמים את הנורמה $\norm{Tv}$ לכל $\norm{v} = 1$. }
	
	\subsection{סיכום תוצאות מרכזיות}
	\subsubsection{סיכום פירוק פרימרי וצורת ג'ורדן}
	...
	\subsubsection{סיכום תבניות בי־לינאריות}
	...
	\end{itemize}
	הבחנו שבמידה והמטריצה המייצגת הרמיטית, אז הסימן של הערכים העצמיים קובע את הסינגטורה (זאת כי פירוק ספקטרלי הוא לא רק דמיון, אלא גם חפיפה!). 
	\subsubsection{סיכום נושא הפירוקים}
	יש לנו מספר סוגי העתקות שעניינו אותנו באופן מיוחד: 
	\begin{center}\begin{tabular}{|c|c|c|c|}
			\hline \textbf{הרמיטית}/\textbf{סימטרית} & \textbf{אורתוגונלית}/\textbf{אוניטרית} & \textbf{נורמאלית} & \textbf{$\C$/$\R$} \\
	...
	\begin{itemize}
		\item \textbf{משפט הפירוק הספקטרלי ב־$\R$: }העתקה היא סימטרית אמ''מ היא לכסינה אורתונורמלית. 
		\item \textbf{משפט הפירוק הספקטרלי ב־$\C$: }אם העתקה היא נורמאלית, אז היא לכסינה אורתונורמלית. 
	...
	\end{itemize}
	למזלנו, ההגדרות מתלכדות במקרה של ההעתקה או מטריצה צמודה לעצמה. זהו המקרה הרלוונטי ל\textbf{פירוק פולארי} שמפרק מטריצה כללית $A$ לכפל של מטריצה אוניטרית $U$ ומטריצה הרמיטית מוגדרת־חיובית $P$ כך ש־$A = UP$ (''לסובב ואז לשנות גודל``). למעשה, לא דיברנו בקורס כלל על מטריצה חיובית (בהבט של $\mut{Av}{v} > 0$) שאיננה צמודה לעצמה, וכאמור במקרה זה היא בכל מקרה מוגדרת־חיובית. 
	
	כבר ידוע שהע''עים של מטריצה צמודה לעצמה (כלומר הרמיטית/סימטרית) הם בהכרח ממשיים. אבל במקרה של מטריצה מוגדרת, נוכל לטעון שמטריצה מוגדרת חיובית אמ''מ הע''עים אף חיוביים (ובאופן דומה לגבי א־שלילית, שלילית, וא־חיובית)!
