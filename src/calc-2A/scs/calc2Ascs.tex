	...
	\begin{multicols}{2}
		\section{עקומות ב־$\R^{2}$}
			...
			\clm{$\cg$ היא עקומה בעלת אורך סופי, וכן $2 \le L(\cg) \le 4$. }\begin{proof}
				...
				פרטים יעלו למודל.
			\end{proof}
			\defi{$\pi := L(\cg)$ כאשר $\cg$ עקומת חצי העיגול ב־$\R^{2}$. }
		...
		\subsection{משפטים נוספים}
			...
			\begin{Theorem}[החלפת משתנה]
				...
				\[ \int^{\phi(b)}_{\phi(a)} \dequad f(x)\dx = \int^{b}_a \dequad \ f(\phi(x))\phi'(x)\dx \]
			\end{Theorem}
		...
		\subsection{טופולוגיה ב־$\R$}
			...
			\begin{Theorem}[הלמה של היינה־בורל]
				נתונה קבוצה סגורה $J$. יהי אוסף $\{U_\ag\}$ (אוסף לאו דווקא בן מניה. פורמלית, אוסף הוא פונקציה $f \co \Lg \to \ps(\R)$ כאשר $U_\ag := f(\ag)$ עבור $\ag \in \Lg$) של קבוצות פתוחות $U_\ag \subset \R$, שמכסות אותו כלומר:
...
%			\end{Theorem}
		...
		\subsection{סכום פורייה}
			...
			\defi{\textit{סכומי פורייה של $f$} הם לכל $n \in \N$:
				\[ (S_nf)(t) := \sum_{\mathclap{n = -N}}^{N}\hat f(n)e^{int} \]}
