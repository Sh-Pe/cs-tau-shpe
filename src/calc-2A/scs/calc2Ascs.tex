...
\titleformat{\section}[block]
	{\fontsize{20}{20}}
	{\Large \dotfill \sen(\thesection)\she \dotfill}
	{0em}
...
\author{\en{{\small By} Shahar Perets \sim \ {\small Based On Lectures By} Yael Karshon}}
\title{\en{Shit Cheat Sheet $\sim$ Calculus 2A $\sim$ TAU}}
...
\begin{document}
	...
	{\vspace{-25pt}\let\newpage\relax\maketitle}\vspace{-20pt}
%	\vspace{-100pt}
	\maketitle
	
	\begin{multicols}{2}
		\section{עקומות ב־$\R^{2}$}
			\defi{בהינתן $a, b \in \R$ וכן $a < b$, \textit{קטע סגור} יוגדר להיות $[a, b] \cod \{c \mid a \le c \le b\}$}
			...
			\defi{ממד העדינות של החלוקה הוא $\lg(\Pi) \cod \max_{i}\Delta x_i$. }
			...
			\begin{Definition}[מרחק אוקלידי]
				\[ d(p, q) \cod \sof{o - q} \cod \sqrt{(x_p - x_q)^{2} + (y_p - y_q)^{2}} \]
			\end{Definition}
			\exe{
			...
			\clm{$\cg$ היא עקומה בעלת אורך סופי, וכן $2 \le L(\cg) \le 4$. }\begin{proof}
				...
				פרטים יעלו למודל.
			\end{proof}
			\defi{$\pi \cod L(\cg)$ כאשר $\cg$ עקומת חצי העיגול ב־$\R^{2}$. }
		\section{אינטגרלי רימן}
			...
			\defi{\textit{חלוקה מתויגת} (באנגלית tagged partition, ברשימות ''חלוקה+נקודות מתאימות``) של הקטע $[a, b]$
				...
				\[ S(f, \pi, \{t_i\}) \cod \sumnio f(t_i)\Delta x_i \]}
			...
			\defi{לפונקציות ממשיות $f, F \co I \to \R$ על קטע $I \subset \R$, אז $F$ נקראת \textit{פונקציה קדומה של $f$ על קטע $I$} אם $\forall x \in I \co F'(x) = f(x)$, כאשר בקצוות הקטע (אם יש כאלו) מדובר על נגזרת חד־כיוונית. }
			...
			\noti{\hfil $R([a, b]) \cod \{f \co [a, b] \to \R \mid \text{אינטגרבילית} f\}$}
		\subsection{דרבו}
			\defi{תהי $f \co [a, b] \to \R$ חסומה. תהי $\Pi = \{x_i\}_{i = 1}^{n}$ חלוקה של $[a, b]$, כלומר $a = x_0 < x_1 < \cdots < x_n = b$. לכל $i$, נגדיר:
				\[ M_i \cod \sup_{\mathclap{[x_{i - 1}, x_i]}} f \quad\quad m_i \cod \inf_{\mathclap{[x{i -1}, x_i]}} f \]
			}
			
			\defi{בהינתן ההנחות מההגדרה הקודמת, נגדיר את \textit{סכום דרבו העליון} ו\textit{סכום דרבו התחתון} להיות:
			\begin{align*}
				\ol{\Sigma}(f, \Pi) = \sum_{i = 1}^{n} M_i\Delta x_i && \dard(f, \Pi) \cod \sum_{i = 1}^{n}m_i \Delta x_i
			\end{align*}}
			\clm{תהי $f \co [a, b] \to \R$ חסומה, ו־$\Pi$ חלוקה של $[a, b]$. אז	לכל תיוג $\{t_i\}$ של החלוקה $\Pi$, מתקיים:
			...
			\defi{תהי $f \co [a, b] \to \R$ חסומה. נגדיר:
				\begin{itemize}
					\item אינטגרל דרבו עליון: \\\hfill$\displaystyle \Idaru(f) \cod \inf \ccb{\daru(f, \Pi) \mid [a, b] \ \text{חלוקה של} \ \Pi}$
					\item אינטגרל דרבו תחתון: \\\hfill$\displaystyle \Idard(f) \cod \sup \ccb{\dard(f, \Pi) \mid [a, b] \ \text{חלוקה של} \ \Pi}$
			\end{itemize}}
			\defi{$f$ \textit{אינטגרבילית דרבו} אם $\Idard(f) = \Idaru(f)$. אם זה קורה, $I(f) = \Idard(f) = \Idaru(f)$ נקרא \textit{אינטגרל דרבו}. }
			\clm{עבור $f \co [a, b]\to \R$, $f$ אינטגרבילית רימן אמ''מ $f$ חסומה וגם $f$ אינטגרבילית דרבו. אם תנאים אלו מתקיימים, אז $\int^{b}_af = I(f)$. }
			...
			\clm{נתונה פונקציה ממשית $f \co [a, b] \to \R$. עבור פונקציה כזו, התנאים הבאים שקולים:
				\begin{enumerate}
					\item קיים $I \in \R$ כך שלכל $\eg > 0$ קיים $\dg > 0$ כך שלכל חלוקה מתוייגת $(\Pi, \{t_i\})$ של $[a, b]$, אם $\lg(\Pi) < \dg$, אז $\sof{S(f, \Pi, \{t_i\}) - I} < \eg$.
					\item הפונקציה $f$ חסומה, וגם לכל $\eg > 0$, קיימת $\dg > 0$ כך שלכל חלוקה $\Pi$ של $[a, b]$, אם $\lg(\Pi) < \dg$, אז $\daru(f, \Pi) - \dard(f, \Pi) < \eg$.
					\item $f$ חסומה, וגם לכל $\eg > 0$ קיימת חלוקה $\Pi$ של $[a, b]$ כך ש־$\daru(f, \Pi) - \dard(f, \Pi) < \eg$
					\item $f$ חסומה, וגם $\Idard(f) = \Idaru(f)$.
			\end{enumerate}}
			\defi{בהינתן $f \co J \to \R$ חסומה, \textit{תנודה} של $f$ ב־$J$ היא $\wg(f, J) \cod \sup_{J}f - \inf_J f$}
			\exe{הוכיחו כי
			...
			\defi{בהינתן $f \co [a, b] \to \R$ חסומה, ו־$\Pi$ חלוקה של $[a, b]$ נגדיר את \textit{תנודת דרבו של $f$ ביחס ל־$\Pi$} (או \textit{התנודה הכוללת של $f$ ביחס ל־$\Pi$}):
				\[ \wg(f, \Pi) \cod \sumnio \wg(f, [x_{i - 1}, x_i])\Delta x_i \]}
			\exe{הוכיחו כי:
			...
			\lem{נתונה $f \co [a, b] \to \R$ חסומה וחלוקות $\Pi, \Pi'$ של $[a, b]$ כך ש־$\Pi'$ מתקבלת מ־$\Pi$ ע''י הוספה של $m_0$ נקודות חלוקה. אז:
				\begin{alignat*}{9}
					(1) \quad\quad 0 &\le \wg(f, \Pi) - \wg(f, \Pi') &\le m_0 \cdot \lg(\Pi) \; \wg(f, [a, b]) \\
					(2) \quad\quad 0 &\le \daru(f, \Pi) - \daru(f, \Pi') &\le m_0 \cdot \lg(\Pi) \; \wg(f, [a, b]) \\
					(3) \quad\quad 0 &\le \dard(f, \Pi) - \dard(f, \Pi') &\le m_0 \cdot \lg(\Pi) \; \wg(f, [a, b])
			\end{alignat*}}
			\clm{תהי $f \co [a, b] \to \R$ חסומה. אז לכל $\eg < 0$ קיימת $\dg > 0$, כך שלכל חלוקה $\Pi$ של $[a, b]$, אם $\lg(\Pi) < \dg$ אז:
				\begin{align*}
					\daru(f, \Pi) - \eg \le \Idaru(f) && \Idard(f) \le \dard(f, \Pi) + \eg
			\end{align*}}
		\subsection{קריטריוני אינטגרביליות נוספים}
			\clm{תהי $f \co [a, b] \to \R$ פונקציה רציפה. אז $f$ אינטגרבילית. }
			\clm{תהי $f \co [a, b] \to \R$ מונוטונית. אז $f$ אינטגרבילית. }
			...
			\clm{$f \co [a, b] \to \R$ חסומה, ו־$a = x_0 < x_1 < \cdots < x_n = b$ חלוקה. נניח שלכל $i$ מתקיים ש־$f|_{(x_{i - 1}, x_i)}$ מונוטונית או רציפה. אז $f$ אינטגרבילית. }
			...
			\defi{נבצע את ההגדרות הבאות בהינתן $a < b$:
				\begin{align*}
					\int^{a}_a\dequad \  f\cod 0 && \int^{a}_b\dequad \  f \cod -\int^{b}_a\dequad \  f
			\end{align*}}
			\clm{נתון קטע $J$ ופונקציה $f \co J \to \R$. נניח שלכל קטע סגור וחסום $I \subset J$ מתקיים $f|_I = I \to \R$. אז לכל $a, b, c \in \R$, מתקיים:
			...
			\begin{Theorem}[לינאריות האינטגרל ביחס לאינטגרנד]
				...
				\[ \int^{b}_a \dequad \ (\ag f + \bg g) = \ag \int^b_a \dequad \ f(x) + \bg \int^b_a \dequad \ f \]
			...
			\begin{Theorem}[רציפות האינטגרל ביחס לגבולות האינטגרציה]
				יהי $I = [\zeta, \xi]$ ותהי $f \co I \to \R$ אינטגרבילית. נקבע $\ag \in I$. נגדיר $F \co I \to \R$ ע''י:
				\[ F(x) \cod \int^x_a \dequad \ f(t)\dt \]
				אז $F$ רציפה.
			\end{Theorem}
			\cola{אם $x\in I$ נקודה פנימית או קצה שמאלית, אז $f$ רציפה מימין ב־$x$. }
			
		\subsection{משפטים נוספים}
			\clm{נתונה פונקציה $f \co [a, b] \to \R$ רציפה על קטע סגור. אז קיימת $a \le x_0 \le b$ כך ש־:
				\[ f(x_0) = \frac{1}{b - a} \int^b_a \dequad \ f(t) \dt \]}
			\begin{Theorem}[משפט ערך הביניים הראשון לאינטגרל מסוים]
				...
				\[ \int^b_a \dequad \ f(t)g(t) \dt = f(x_0)\!\int^{b}_a\dequad \ g(t) \dt \]
			\end{Theorem}
			...
			\end{Lemma}
			\begin{Theorem}[המשפט היסודי של החדו''א]
				תהי $f \in R([a, b])$ ותהי $x_0 \in [a, b]$ נקודת רציפות של $f$. אזי $F(x) = \int^{x}_a f(t)\dt$ גזירה ב־$x_0$ ומתקיים $F'(x_0) = f(x_0)$
			\end{Theorem}
			\begin{Theorem}[ניוטון־לייבניץ]
				תהי $f \in R([a, b])$ ותהי $F \co [a, b] \to \R$ קדומה של $f$ ב־$[a, b]$. אזי: 
				\[ \int^{b}_a \dequad \ f(x)\dx = F(b) - F(a) \]
			\end{Theorem}
			\begin{Theorem}[החלפת משתנה]
				תהי $f \co [a, b] \to \R$ רציפה ותהי $\phi \co [a, b] \to [\phi(a), \phi(b)]$ גזירה ברציפות. אז: 
				\[ \int^{\phi(b)}_{\phi(a)} \dequadd\ f(x)\dx = \int^{b}_a \dequad \ f(\phi(x))\phi'(x)\dx \]
			\end{Theorem}
			\begin{Theorem}[אינטרציה בחלקים]
				יהיו $f, g \co [a, b] \to \R$ גזירות ונגזרותיהן אינטגרביליות. אז: 
				\[ \int^{b}_a\dequad \ f'(x)g(x)\dx = \csb{f(x)g(x)}\bigg\vert^{b}_a \!- \int^{b}_a\dequad f(x)g'(x)\dx \]
			\end{Theorem}
			...
			\defi{''עצרת בדילוגים`` (אנ' double factorial): עבור $k \in \Neven$ נגדיר $k!! = 2 \cdot 4 \cdots (k - 2) \cdot k$, בעבור $k \in \Nodd$ נגדיר $k!! \cod 1 \cdot 3 \cdot 5 \cdots (k- 2) \cdot k$, ועבור $k = -1, 0$ נגדיר $k!! = 0$. }
			\clm{\[ \int^{2\pi}_0 \dequad \sin^{m}x \dx = \begin{cases}
		...
		\subsection{האינטגרל הלא אמיתי ותכונותיו}
			...
			\defi{נתונה $f \co \R \to \R$ ונתון $I \in \R$. נאמר ש\textit{האינטגרל הלא אמיתי של $f$ מתכנס ל־$I$} אם $\int^{\infty}_0 f(x) \dx, \int^{0}_{-\infty}f(x)\dx$ מתכנסים, ומתקיים:
				\[ \int^{\infty}_{-\infty}\dequad\! f(x)\dx \cod \int^{0}_{-\infty}\dequad\! f(x)\dx + \int^{\infty}_0 \dequad f(x)\dx = I \]}
			...
			\end{Theorem}
			\begin{Theorem}[אינטגרציה בחלקים לאינטגרלים לא אמיתיים]
				...
				כלומר:
				אם שני הגבולות מימין מתכנסים, אז הגבול משמאל מתכנס ומתקיים השוויון. 
			\end{Theorem}
			\begin{Theorem}[התכנסות מונוטונית]
				נתונה $g \co [a, \wg) \to \R$ מונוטונית חלש.
				\begin{itemize}
					\item אם $g$ עולה וחסומה, אז: \hfill $\disty \smash{\lim_{b \to \wg^{-}}} g(b) = \smash{\sup_{\hopen}} g$
					\item אם $g$ עולה אך לא חסומה, אז: \hfill $\disty \smash{\lim_{b \to \wg^{-}}} g(b) = \infty$
					\item אם $g$ יורדת וחסומה, אז: \hfill $\disty \smash{\lim_{b \to \wg^{-}}} g(b) = \smash{\inf_{\hopen}} g$
					\item אם $g$ יורדת אך איננה חסומה, אז: \hfill $\disty \smash{\lim_{b \to \wg^{-}}} g(b) = -\infty$
				\end{itemize}
			\end{Theorem}
			...
			\end{Theorem}
			\clm{תהי $g \co \hopen \to \R$ חסומה $m \le g \le M$. נגדיר $h, H \co \hopen \to \R$ ע''י:
				\[ h(b) = \int_{[b, \wg)} \dequad \! g(x) \quad\quad H(b) = \sup_{[b, \wg)} g(x) \]
			...
			\defi{נתונה $g \co \hopen \to \R$. אז:
				\begin{gather*}
					\limsup_{b \to \wg^{-}} g(x) = \ol{\lim_{x \to \wg^{-}}} g(x) \cod \bto {\sup_{[b, \wg)} g(b)}  = \inf_{b \to \hopen} \sup_{\hopen} g
				\end{gather*}
		...
		\subsection{קריטריוני התכנסות}
			\begin{Theorem}[קריטריון קושי]
				נתונה $f \co \hopen \to \R, f \in R([a, b])$ לכל $b \in [a, \wg)$. אז האינטגרל $\int^{\wg}_a f(x)\dx$ מתכנס אמ''מ לכל $\eg > 0$ קיימת $a < b_0 < \wg$ כך שלכל $b_1, b_2 \in [b_0, \wg)$ מתקיים:
			...
			\clm{נתונה $f \co [1, \infty) \to \R$.
				...
				נניח $f$ יורדת במובן החלש ו־$f > 0$. אז האינטגרל הלא אמיתי $\int^{\infty}_1 f(x)\dx$ מתכנס אמ''מ הטור $\sumninf f(n)$ מתכנס. ואם תנאים אלו מתקיימים, אז $\sum_{n = 2}^{\infty}f(n) \le \int^{\infty}_1 f(x) \dx \le \sumninf f(n)$.
			}
			...
			\begin{Theorem}[השוואה לטורים]
				תהי $f \co [1, \infty) \to \R$ יורדת ואי־שלילית. אזי האינטגרל $\int^{\infty}_1 f(x)\dx$ מתכנס אמ''מ הטור $\sum_{n = 1}^{\infty}f(n)$ מתכנס, וכן: 
				\[ \sum_{n = 2}^{\infty}f(n) \le \int^{\infty}_1\dequad f(x)\dx \le \sum_{n = 1}^{\infty}f(n) \]
			\end{Theorem}
			
			\begin{Theorem}[קריטריון ההשוואה]
				יהיו $f, g \co [a, \wg) \to \R$ פונקציות אי־שליליות אינטגרביליות בכל תתי־הקטעים. נניח $f \le g$. אז $\int^{\wg}_a f(x)\dx$ מתכנס אם $\int^{\wg}_a g(t)\dx$ מתכנס, ואם $\int^{\wg}_a f(x)\dx$ מתבדר אז $\int^{\wg}_a g(t)\dx$ מתבדר. 
			\end{Theorem}
		...
		\section{סדרות פונקציות}
		\subsection{הגדרות}
			\defi{יהי $I \subset \R$ קטע, $f \co I \to \R$ פונקציה ו־$f_n \co I \to \R$ סדרת פונקציות. נאמר ש־\textit{$f_n$ מתכנסות במידה שווה ל־$f$} ונסמן $f_n \tou f$ אם: 
			\[ \sup_{x \in I}\sof{f_n(x) - f(x)} \toinf 0 \]}
			\defi{נאמר ש־$f_n \co I \to \R$ סדרת פונקציות \textit{חסומה במידה אחידה} אמ''מ קיימת $M \in \R$ כך ש־$\sof{f_n} < M$ לכל $n \in \N$. }
			\begin{Claim}[אריתמטיקה של התכנסות במ''ש]
				יהיו $f_n, g_n \co I \to \R$ סדרות פונקציות ונניח $f_n \tou f, g_n \tou g$. אז: 
				\begin{itemize}
					\item \hfil $(f_n + g_n) \tou f + g$
					\item אם $f_n, g_n$ חסומות במידה אחידה, אז $f_ng_n \tou fg$. 
				\end{itemize}
			\end{Claim}
			\begin{Theorem}[קריטריון קושי במ''ש]
				תהי $f_n \co I \to \R$. אז $f_n$ מתכנסות במ''ש לפונקציה כלשהי אמ''מ: 
				\[ \forall \eg > 0.\, \exists N_\eg .\,\forall n, m > N_\eg .\, \forall x \in I \co \sof{f_n(x) - f_m(x)} < \eg \]
			\end{Theorem}
		\subsection{משפטים}
			...
			\begin{Theorem}[גבול תחת אינטגרל]
				...
				\[ F_n(x) \cod \int^{x}_a \dequad \ f_n(t)\dt \quad \quad F(x) \cod \int^{x}_a \dequad \ f(t)\dt \]
				מקיימות $F_n \to F$ במ''ש.
			\end{Theorem}
			\begin{Theorem}[משפט המג'וראנטה]
				יהיו $f, f_n \co [a, \infty) \to \R$. נניח ש־$f_n \to f$ במ''ש על כל ת''ק סגור וכן לכל $n$, $f_n$ אינטגרבילית על כל ת''ק סגור. (לכן $f$ אינטגרבילית גם היא על כל תת־קטע סגור). נניח שקיימת $\Psi \co [a, \infty) \to \R$ (''המאז'וראנטה``) כך ש־:
				\begin{enumerate}
			...
			\clm{נתונות $f, g, f_n \co [a, b] \to \R$ ונניח ש־$f_n \in C^{1}([a, b])$ נקודתית וכן $f_n \to f, f_n' \tou g$. אז $f_n \tou f$ במ''ש וגם $f' = g$. }
			\begin{Theorem}[החלפת גבול בנגזרת, גרסה כללית יותר]
				יהיו $f_n, \ g \co [a, b] \to \R$. נניח:
				\begin{itemize}
					\item $f_n \in C^{1}([a, b])$ גזירה ברציפות. 
					\item $f_n' \tou g$ במ''ש. 
					\item קיימת $x_0 \in [a, b]$ כך ש־$(f_n(x_0))^{\infty}_{n = 1}$ מתכנסת ל־$c \in \R$.
				\end{itemize}
				אז $f_n$ מתכנס במ''ש לפונקציה:
				\[ f_n\overset{u}{\to} f(x) \cod c + \int^{x}_{x_0} \dequad \ g(t)\dt \]
		...
		\subsection{טופולוגיה ב־$\R$}
			...
			\defi{קבוצה $K \subset \R$ נקראת \textit{סגורה} אם המשלים שלה $X \setminus K$ פתוח. }
			
			\begin{Theorem}[הלמה של היינה־בורל]
				נתונה קבוצה סגורה $J$. יהי אוסף $\{U_\ag\}$ (אוסף לאו דווקא בן מניה. פורמלית, אוסף הוא פונקציה $f \co \Lg \to \ps(\R)$ כאשר $U_\ag \cod f(\ag)$ עבור $\ag \in \Lg$) של קבוצות פתוחות $U_\ag \subset \R$, שמכסות אותו כלומר:
				\[ J \subset \bigcup_\ag U_\ag \]
				אז קיים תת־אוסף סופי $U_{\ag_1} \dots U_{\ag_n}$ שמכסה אותו, כלומר $J \subset U_{\ag_1} \cup \cdots \cup U_{\ag_n}$.
			\end{Theorem}
			
			\cola{קבוצות ב־$\R$ הן קומפקטיות אמ''מ הן סגורות וחסומות. }
		...
		\subsection{הגדרות בסיסיות}
			...
			\clm{אם טור מתכנס בהחלט במ''ש, אז $\sumninf \sof{u_n(x)}$ מתכנס לכל $x$ וגם $\sumninf u_n(x)$ מתכנס במ''ש (הוכחה באמצעות קריטריון קושי במ''ש). \textbf{הכיוון השני לא עובד. }התכנסות בהחלט + התכנסות במ''ש לא גורר התכנסות בהחלט במ''ש. }
			
		\subsection{על התכנסות טור פונקציות}
			\clm{נניח $\sumninf u_n(x) =f(x)$ במ''ש ב־$I = [a, b]$.
				\begin{enumerate}
			...
			\clm{תחת ההגדרות: 
				\begin{multline*}
					u_0(x) \cod \begin{cases}
						\{x\} & n \le x \le n + \frac{1}{2} \\
						1 - \{x\} & n + \frac{1}{2} \le x \le n + 1
					\end{cases} \\ u_k(x) \cod \smash{\frac{1}{4^{k}}u_0(4^{k}x)}
				\end{multline*}
				...
				\[ \frac{f(x_n) - f(x_0)}{x_n - x_0} \in \begin{cases}
					...
					\Nodd & n \in \Nodd
				\end{cases} \]}
			\cola{הטור $f(x) \cod \sumkoinf u_k(x)$ מתכנס במ''ש לפונקציה שאיננה גזירה באף נקודה. }
			
			
			\begin{Theorem}[וויראטשראס]
				תהי $f \co C([a, b])$ כלשהי. אז קיימת סדרה של פולינומים $P_k(x)$ כך ש־$P_k \to f$ במ''ש. 
			\end{Theorem}
		...
		\subsection{טורי חזקות}
			...
			\lem{נניח $\sum a_k x^{k}$ מתכנס ב־$x_0 \neq 0$. יהי $R \cod \sof{x_0}$. יהי $0 < R' < R$, אז הטור מתכנס במ''ש בקטע $[-R', R']$. }
			\rmark{שימו לב, הטור לאו דווקא מתכנס בקטע הפתוח $(-R, R)$. }
			\cola{יהי נתון טור חזקות $\sumkoinf a_kx^{k}$. אז:
				\begin{enumerate}[(A)]
					\item הטור מתכנס ב־$(-R, R)$. נוכל להגדיר $f \co (-R, R) \to \R$ ע''י $f(x) = \sumkoinf a_kx^{k}$. מעתה ואילך, נגדיר $f(x) \cod \sumkoinf a_kx^{k}$.
					\item $f$ רציפה.
					\item גם הטור $\sumkoinf a_k\frac{x^{k + 1}}{k + 1}$ מתכנס ב־$(-R, R)$, ולכל $x \in (-R, R)$ מתקיים:
					\[ \int_0^{x}\dequad \ f(t)\dt = \sumkoinf \frac{x^{k}}{k + 1} \]
			\end{enumerate}}
		
			\begin{Theorem}[בוחן $M$ של וויראשטראס]
				תהי $u_n \co I \to \R$ ונניח שקיימת סדרת מספרים $M_n$ המקיימת $\sum_{n = 1}^{\infty}M_n < \infty$, כל שלכל $n$ מתקיים $\sof{u_n(x)} < M_n$. אזי $\sum_{i = 1}^{\infty}u_n$ מתכנס בהחלט ובמ''ש. 
			\end{Theorem}
			...
			\begin{Theorem}[אבל]
				...
				\end{itemize}
				ה־$R$ הזה נקרא \textit{רדיוס התכנסות}.
			\end{Theorem}
		...
		\subsection{גזירה איבר־איבר של טור חזקות}
			\clm{לטורים $\sumkoinf a_kx^{k}$ ו־$\sumkinf ka_kx^{k - 1}$ יש אותו רדיוס ההתכנסות. }
			\clm{נתון טור חזקות $\sumkoinf a_k x^{k}$, עם רדיוס התכנסות $R$. נגדיר $f \co (-R, R) \to \R$ ע''י $f(x) = \sumkoinf a_kx^{k}$. אז $f$ גזירה ב־$(-R, R)$ ו־$\frac{\dd f}{\dx} = \sumkinf ka_kx^{k - 1}$. }
			\clm{נניח $f(x) = \sumkoinf a_kx^{k}$ ב־$(-R, R)$. אז $f$ גזירה אינסוף פעמים ב־$0$ ו־$\sumkoinf a_kx^{k}$ הוא טור טיילור של $f$ ב־$x = 0$. }
		...
		\subsection{קצה תחום ההתכנסות}
			\clm{נתון טור חזקות $\sumkoinf a_kx^{k}$ עם רדיוס התכנסות $0 < R \in \R$. אם הטור מתבדר ב־$x = R$, אז ההתכנסות של הטור ב־$[0, R)$ איננה במ''ש. }
			\begin{Theorem}[משפט אבל על קצה תחום ההתכנסות]
				נניח $f(x) = \sumkoinf a_kx^{k}$, ונניח ש־$f$ מתכנס נקודתית ב־$R$. אז התכנסות הטור ל־$f(x)$ היא במ''ש.
			\end{Theorem}
			\begin{Theorem}
		...
		\subsection{סוגי סכימות}
			\begin{Definition}[סוגי סכימות]
				\begin{enumerate}
					...
					\item \textit{צזארו לטורים: }טור $\sum_{n = 0}^{\infty}a_n$ \textit{מתכסנת צזארו ל־$L$} אמ''מ סדרת הסכומים החלקיים מתכנסת צזארו ל־$L$. נסמן $\cz \sum_{n = 0}^{\infty}a_n = L$. 
					\item \textit{אבל: }טור פורמלי $\sumkoinf a_k$ \textit{מתכנס לפי אבל ל־$K$} אם $\lim_{R\to 1^{-}} \sumkoinf a_kR^{k} = L$. נסמן $\ab a_n \to L$
				\end{enumerate}
			\end{Definition}
			\defi{בהינתן סדרה (או טור) $(a_n)_{n = 1}^{\infty}$, נגדיר את \textit{סכומי אבל} שלה להיות $\sg_N = \frac{\sum_{n = 0}^{N}a_n}{N + 1}$. }
		
			\rmark{בעבור התכנסות צזארו של טורים, למעשה יש לנו סכום כפול: 
				\[ \sg_N = \frac{1}{N + 1}\sum_{n = 0}^{n}{\sumnko a_k} = \sum_{k = 0}^{N}\cl{1 - \frac{k}{N + 1}}a_k \]
			}
			\clm{אם טור $\sumkoinf a_k = L$ במובן הרגיל, אז $\ab \sumkoinf a_k = L$ וגם $\cz \sumkoinf a_k = L$. }
			\clm{התכנסות צזארו גוררת התכנסות לפי אבל, כלומר אם $\cz \sumkoinf a_k= L$, אז $\ab \sumkoinf a_k = L$. }
			\begin{Theorem}[טאובר]
				אם $\ab \sumkoinf a_k$ וגם $a_k = o(\frac{1}{k})$ אז $\sumkoinf a_k = L$.
			\end{Theorem}	
			\clm{אם טור מתכנס בהחלט, אז הוא מתכנס, וכמו כן כל טור שמתקבל ממנו ע''י שינוי סדר האיברים או קיבוץ האיברים, גם הוא מתכנס (בהחלט) לאותו הגבול. }
			...
			\clm{\begin{enumerate}
					...
					\begin{enumerate}[(A)]
						...
						\item לכל פירוק זר לתתי קבוצות סופיות $\Z_{\ge 0} = J_0 \uplus J_1 \uplus J_2 \uplus \cdots$, אם נסמן $c_m\cod \sum_{j \in J_m} t_j$ הטור $c_m$ מתכנס בהחלט ל־$L$.
					\end{enumerate}
			\end{enumerate}}
			\clm{נתונים טורים $\sum_{\ml = 0}^{\infty}\bg_{\ml}$ ו־$\sum_{k = 0}^{\infty}\ag_k$ שמתכנסים בהחלט לגבולות $A, B$ בהחלפה. נגדיר $\cg_m = \sum_{k + \ml = m}\ag_k \bg_\ml$. אז הטור $\sum_{m = 0}^{\infty}\cg_m$ שנקרא \textit{מכפלת קושי של הטורים $\sum \ag_k, \sum \bg_k$} מתכנס בהחלט לגבול $A \cdot B$. }
		\section{פורייה}
%		\subsection{מרוכבים}
...
%				תהי $f \co [a, b] \to \C$ רציפה ב־$x_0$. אז $F \co [a, b] \to \C$ המוגדרת ע''י $F(x) \cod \int^{x}_a f(t)\dt$ מקיימת $F'(x_0) = f(x_0)$.
%			\end{Theorem}
%			\begin{Theorem}[ניוטון לייבניץ]
...
%			\end{Theorem}
		...
		\subsection{$R(\tb)$ ותתי־מרחביו}
			...
			\lem{\begin{multline*}
					\Sp_\C\cl{\ccb{e^{int} \mid -N \le n \le N}} \\= \Sp_\C\cl{\{1\} \cup \{\cos nt\}_{n = 1}^{N} \cup \ccb{\sin t}_{n = 1}^{N}}
			\end{multline*}}
			
			\exe{אם $f, g \in R(\tb)$, אז גם $f \bar g \in R(\tb)$. }
			\defi{\[ \mut{f}{g} \cod \frac{1}{2\pi}\int^{2\pi}_0 \dequad f(t)\ol{g(t)}\dt \]}
		...
		\subsection{סכום פורייה}
			\defi{\textit{מקדמי פוריה} (Fourier) של $f \in R(\tb)$, הם לכל $n \in \Z$:
				\[ \hat f(n) \cod \frac{1}{2\pi}\int^{\pi}_{-\pi}\dequad f(t)e^{-int}\dt = \mut{f}{e^{int}} \]}
			\defi{\textit{סכומי פורייה של $f$} הם לכל $n \in \N$:
				\[ (S_nf)(t) \cod \sum_{\mathclap{n = -N}}^{N}\hat f(n)e^{int} \]}
			\defi{\textit{טור פורייה של $f$} הוא:
				\[ \sum_{n = - \infty}^{\infty}\dequad \ \hat f (n) e^{int} \]}
			
			\clm{תהי $f \in R(\tb)$, אז $S_nf$ הוא ההטלה האורתוגונלית של $f$ על מרחב הפולינומים הטריגונומטריים מדרגה $n\ge $. כלומר: 
			\begin{enumerate}[(1)]
		...
		\subsection{טור פורייה}
			...
			\end{Theorem}
			\clm{תהי $f \in C(\tb)$, ונניח ש־$\sum^{\infty}_{n = -\infty}\sof{\hat f(n)} < \infty$ (שימו לב – לא ריבועי מקדמי פורייה, אלא ממש מקדמי פוריה עצמם). אז $S_n f \tou f$ במ''ש. }
		...
		\subsection{גרעין אבל ודיריכלה}
			\defi{לכל $f, g \in R(\tb)$, ה\textit{קונבלוציה של $f, g$ בנקודה $x$} היא:
				\[ (f * g)(x) \cod \frac{1}{2\pi}\int^{\pi}_{-\pi}\dequad f(t)g(x - t)\dt \]}
			\begin{Claim}[תכונות הקונבלוציה]\
				\begin{itemize}
					...
					\item באופן יותר ספציפי, מתקיים ש־$\widehat{(f * g)}(n) = \hat f (n) \hat g(n)$. 
				\end{itemize}
			\end{Claim}
			...
			\begin{Lemma}[נוסחה מפורשת לגרעין דיריכלה]
				\[ D_N(y) = \sum_{\mathclap{n = -N}}^{N}e^{iny} = \begin{cases}
					2N + 1 & y = 0 \\
			...
			\begin{Lemma}[נוסחה מפורשת של גרעין פייר]
				\[ F_N(y) = \frac{1}{N + 1}\sum_{n= 0}^{N}D_n(y) = \frac{1}{N + 1} \cl{\frac{\sin \cl{\frac{N + 1}{2} y}}{\sin(\frac{1}{2}y)}}^{2} \]
			\end{Lemma}
	...
	\hfil \en{\textit{Compilation Date:} \today}
	
	\vfil
	\renewcommand{\contentsname}{\vspace{-60pt}}
	\begin{multicols}{2}
		\tableofcontents
	\end{multicols}
