...
\begin{document}
	...
	\[ \hat f(n) := \frac{1}{2\pi}f(t)e^{-int}\dt = \mut{f}{e^{int}} \]}
	...
	\end{proof}
	\rmark{זוהי מסקנה יותר כללית מלינארית א2 שנכונה לכל מכפלה פנימית. }
	...
	\[ \lim_{n \to \infty}\norm{f_n - f}_{L_2} = 0 \]}
	\rmark{שימו לב שזהו מרחב מטרי, ולכן הוא first countable (מקיים $C_1$). זה למה בכלל יש פואנטה לדבר על סדרות. }
	...
	\lem{נתונה $f_n \to f \in C(\tb) \subset R(\tb)$. נניח $f_n \toinf f$, רז $f_n \to f$ ב־$L_2$. }
	\begin{proof}
		''ההוכחה קלה``
	...
	\rmark{שימו לב שאיברי הטור אי־שליליים ולכן סדר הסכימה אינו משנה. }\begin{proof}
		לכל $N_1, N_2$ מתקיים: 
		\[ \sum_{n = -N_1}^{N_2}\sof{\hat f(n)}^{2} \le \sum_{n = -N}^{N} \seq \norm{S_Nf}^{2} \overset{*}{\le} \norm{f} \]
