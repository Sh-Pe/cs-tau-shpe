...
\author{שחר פרץ}
\title{מ.מ.למדמ"ח $\sim$ עמית ויינשטין $\sim$ עיבוד תמונה}

\begin{document}
	\maketitle
	
	\section{\en{Noise}}
	\subsection{סוגי לכלוך}
	\subsubsection{Salt \& Pepper}
	לכל פיקסל באופן בלתי תלוי, בהסתברות $p$ כלשהי, נדרוס אותו עם ערך $0$ או $1$ בצורה אקראית. 
	\subsubsection{Gaussian Noise}
	ניקח מספר $p$ שנמפלג לפי התפלגות גאוס (התפלגות נורמלית). 
	
	\subsection{פתרון}
	\subsubsection{Local Mean}
	\textbf{הצעה: }עבור כל פיקסל (מלוכלך, במקרה שלנו), נחליף אותו בערך הצבע הממוצע של השכנים שלו
	
	כאשר יש בעיות כמו רעש גאוסיאני, נעשה את זה לכל הפיקסלים. 
	\subsubsection{Local Median}
	
	\textbf{הצעה 2: }אותו הדבר רק עם חציון. 
