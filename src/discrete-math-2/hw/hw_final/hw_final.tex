...
\begin{document}
	...
	\begin{enumerate}[(A)]
		...
		\item \textbf{שאלה: }מצאו נוסחת נסיגה ל־$F(n)$ ללא שימוש ב־$D(n)$ או בסימן סכום. 
		
		\textbf{תשובה: }נוכיח $D(n) = (n - 1)(D_{n - 1} + D_{n - 2})$. נתבונן בסדרה בעלת $n$ איברים. נבחר את האיבר הראשון בה, נסמנו $a_1$. יהיו $n - 1$ אפשרויות לבחור מספר שהיא תפנה אליו (כמות האפשרויות, פחות היא עצמה), נסמנו $a_j$. אם $a_j$ מפנה אליה, אז סה"כ ידועות לנו שתי הפניות ויהיו $D_{n - 2}$ אפשרויות – כמות האפשרויות לסדר את כל השאר. אם לא, אז נתבונן בכמות האפשרויות לסדר את $n - 1$ האיברים כולל $a_j$, שהיא $D_{n - 1}$, אך על כל סידור נגדיר הרכבה ב־$j \to 1$, כי כבר ידוע ש־$j$ מופנה ע"י 1. סה"כ שני הביטויים קשורים בבחירת $j$, כלומר נקבל $(n - 1)(D_{n - 1} + D_{n - 2})$ כדרוש. 
		
		סה"כ נציב בנוסחה מסעיף (א) ונקבל נוסחת נסיגה שלא תלויה בשום דבר פרט לעצמה. כתנאי התחלה, $F(0) = 1$. 
	\end{enumerate}
	\section{}
	\begin{enumerate}[(A)]
	...
	\begin{proof}
		יהיו $a_i, b_i$ סדרות כמתוארות להלן. אזי: 
		\[ |I| \le \sum_{i \in I} 1 \le \sum_{i \in I}a_i \le \sum_{i \in I} n = |I|n \]
			ובאופן דומה על $b_i$. נסמן $|I| = k$, ונדע $I \subseteq [2n]$ כלומר $0 \le I \le 2n$. לכן, כמות האפשרויות ל–$|I|$ תהיה $2n^k$. וסך האפשרויות בעבור כל $k$, יהיה:
			\[ S_1 := \sum_{k = 1}^{2n} 2n^k = \frac{2n^{1} - 2n^{2n + 1}}{1 - 2n} = \frac{2n(2n^{2n} - 1)}{2n - 1} = (2n - 1)(2n^{2n} - 1) - \frac{2n^{2n} - 1}{2n - 1}\]
			בעוד כמות האפשרויות לאידקסים, בעבור כל $k$: 
			\[ S_2 := \sum_{k = 1}^{2n} k(n - 1) = (n - 1) \frac{2n(2n + 1)}{2} = (2n + 1)\frac{2n(n - 1)}{2} \]
			כאשר היחס ביניהם: 
			\[ \frac{S_1}{S_2} = 2\frac{\frac{2n(2n^{2n} - 1)}{2n - 1}}{2n(n - 1)(2n - 1)} = 2\frac{(2n^{2n} - 1)}{(2n - 1)^{2}(n - 1)} \ge 2 \]
			ולכן מעקרון שובך היונים המורחב, קיימים שתי אפשרויות שונות לאינדקסים, בעבור אותו הסכום, כלומר בהכרח קיימים $I, J$ מתאימים. 
	\end{proof}
...
%		TODO: להוכיח קיום
	...
	\begin{enumerate}[(A)]
		...
		\end{proof}
		\item צ.ל. $R(4, 4) \le 18$. \begin{proof}
			בהרצאה הוכח כי $R(2, k) = k$ וגם $R(3, 3) = 6$. נתבונן בא"שים הבאים (נסתמך על מה שהוכחנו למעלה): 
			\[ R(3, 4) = R(4, 3) = R(3, 3) + R(2, 4) - 1 = 6 + 4 - 1 = 9 \]
			ולכן, מהא"ש $R(s, t) = R(s - 1, t) + S(s, t - 1)$: 
			\[ R(4, 4) \le R(4, 3) + R(3, 4) = 9 + 9 = 18 \]
			כדרוש. 
		\end{proof}
	\end{enumerate}
	\section{}
	...
	\begin{enumerate}[(A)]
		...
		\begin{proof}
			נתבונן בגרף $G - v = \la \tl V, \tl E \ra$. נניח בה"כ ש־$c(v) = 1$ (עד לכדי זיווג $[n] \to \Img(c)$). נתבונן בקשת $\{v, w\}$ שהוסרה, ונניח $c(w) = 2$. אם קיימת קשת $\{v, \bar w\}$ כך ש־$c(\bar w) = 2$, אז לא יהיה אפשר לחבר את הצביעה של הצמתים בצבע $1$ אם אלו בצבע $2$, ושאר הצבעים לא שונו ולכן הצביעה המינימלית עודנה $\chi(G)$. אחרת, אין שום קשת בין צומת מצבע $1$ לצומת מצבע $2$ – ולכן אפשר ליצור פונקציה $\tl c$ שתגדיר: 
			\[ \begin{cases}
				x \mapsto x, \ c(x) \neq 2\\
				x \mapsto 1, \ \other
			\end{cases} \]
			שתהיה פונקציית צביעה חוקית מסדר של $\chi(G) - 1$. לא ייתכן פחות מכך, כי כל שאר קבוצות הצבעים האחרות נותרו עם אותם הצמתים והקשתות בדיוק. 
			
			\textit{(יש חור קטן בהוכחה שלא הצלחתי לפתור, בנוגע למה קורה אם משנים לחלוטין את סדר הצביעה. אשמח אם יסבירו בהרצאה או בתרגול איך לסגור אותו)}
			
		\end{proof}
		\item צ.ל. $\chi(G) + \chi(\ol G) \le |V| + 1$. 
		
		\textit{לא הצלחתי לפתור את הסעיף, ולא ידוע לי על מישהו אחד שפתר את הסעיף. }
	\end{enumerate}
	\section{}
