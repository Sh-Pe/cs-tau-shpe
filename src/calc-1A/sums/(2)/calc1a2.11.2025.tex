...
%		\envendproof
	...
	\defi{אם $\an$ מתכנסת וגבולה (היחיד) הוא $\ml$, נסמן $\lim_{n \to \inf} a_n = \ml$. }
	...
	\theo{תהא $\an$ סדרה. יהי $\ml \in \R$. נניח כי $\ml \neq 0$ וגם $\limani = \ml$. אז קיים $N \in \N$ כך שלכל $n \ge N$ מתקיים $\sof{a_n} \ge \frac{\sof{\ml}}{2}$. }
	...
	\begin{proof}
		ידוע $\ml \neq 0$ ולכן $\sof{\ml} > 0$. דהיינו $\frac{\sof{\ml}}{2} > 0$. אז עבור $\eg = \frac{\sof{\ml}}{2}$ קיים $N \in \N$ כך שלכל $n \ge N$ מתקיים $\sof{a_n - \ml} < \frac{\sof{\ml}}{2}$. נתבונן ב־$n$. יהי $n \ge N$' tz:
		...
		\begin{itemize}
			...
			\item נניח $\ml < 0$. נניח $\ml < 0$. ואז $\an < \ml + \frac{\sof{\ml}}{2} = - \frac{\sof{\ml}}{2} < 0$. ולכן $\sof{a_n} > \frac{\sof{\ml}}{2}$
		\end{itemize}
		אפשר גם להוכיח עם א''ש המשולש, זה יוצא יותר קצר, אבל good luck with that. 
	\end{proof}
	
	\rmark{הערה לעצמי: לעבור על ההוכחה למעלה כשאני ערני. ותוכיח את הטענה לעיל עם א''ש המשולש. }
	...
	\theo{תהאנה $\an, \bc$ סדרות. יהיו $\ml, m \in \R$ ממשיים. נניח כי $\limani = \ml \land \limbni = m$. אז: 
	\begin{enumerate}
	...
	\begin{proof}[הוכחה עבור 3]
		[טיוטה: $\sof{a_n b_n - \ml m} = \sof{a_n b_n - a_n m - a_n m - \ml m} \le \sof{a_n b_n - a_n m} + \sof{a_n m - \ml m} = \sof{a_n}\sof{b_n - m} + \sof{m}\sof{a_n - \ml}$ ואז נקבל $\sof{a_n - \ml} < \frac{\eg}{2(\sof{m} + 1)}$, ולגבול השני נבחר חסם בהתאם לגבול]
	...
	לבית תוכיחו את כל השאר. ע''מ הקל עליכם, אפשר להוכיח ב־4 עבור $\frac{1}{b_n}$ ואז להשתמש ב־3. 
	
	\defi{תהא $\an$ סדרה. נאמר כי $\an$ שואפת ל־$+\inft$ כאשר: \hfill $\forall M > 0.\, \exists N \in \N.\, \forall n \i \ge N \co a_n > M$}
	\defi{תהא $\an$ סדרה. אנרמ כי $\an$ שואפת ל־$-\inft$ כאשר: \hfill $\forall M > 0.\, \exists N \in \N.\, \forall n \ge N \co a_n < -M$}
	
	\theo{תהיינה $\an, \bn$ סדרות. נניח $\limani = +\inft \land \limbni b_n = + \inft$. אז $\limsi a_n + b_n = + \inft$}
	\begin{proof}
