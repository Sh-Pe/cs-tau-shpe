%  CALC 1A SUMMARY ~ 2025B ~ by Shahar Perets ~ original .TeX
%  file ~ compile with LuaLaTeX
% 
%  Copyright (C) 2026  Shahar Perets
%  
...
%! ~~~ Packages Setup ~~~
\documentclass[name=רשימות חשבון דיפרנציאלי ואינטגרלי 1א]{../../../tex/classes/megasumPreamble}
...
\begin{document}
	...
	להלן מובא סיכום לקורס חדו''א 1א, כפי שעובר ע''י ליאור קמה לתלמידי אודיסאה מסלולי סייבר ופיזיקה, בסמסטר א' של 2026. 

	ניוטון פיתח לראשונה את החדו''א ככלי לנתח ככובי לכת וקליעים. תוך כדי כך לייבניץ פיתח את החדו''א. ההגדרות לא היו פורמליות בכלל. זה השתנה לאחר פרדוקס ראסל, ולאחרי שזרם הפורמליזם של הילברט בגטינגן השתלט על הכל. ואז היה נאציזם ואז הפסיקה להיות מתמטיקה בגטינגן. 
	
	הקורס סובב סביב שני פרקים עיקריים: סדרות, ופונקציות. בהתחלה, נדבר על סדרות – האם ולאן סדרות שואפות באינסוף. כאשר נלמד על פונקציות, יסתבר לנו שחקר של התכנסות סדרות מאפשר לנו להגדיר התכנסות פונקציות (משפט היינה). זה משפט לא טרוויאלי בכלל, ואם תקחו טופולוגיה יסתבר לכם שזה מתקיים רק במרחבים שמקיימים מה שנקרא \textit{אקסיומת מנייה ראשונה} (למעשה, מרבית התוצאות שנוכיח על סדרות גם בחדו''א 2א מתקיימות רק בהקשר זה). קשר יותר כללי, בין רשתות (הכללה של סדרות) לבין פונקציות רציפות, מתקיים גם במרחבים טופולוגיים כלליים. 
	
	ליאור הרחיב ודיבר מעט על הטופולוגיה של הישר הממשי, כדי לספק הגדרות כלליות יותר למושג הגבול על $\R$. החומר הזה לא מועבר בקורס הרגיל ומובא כאן כהעשרה בלבד. 
	...
	\newpage
	\section[הישר הממשי]{\en{The $\R$eal Line}}
...
אז $\ml \le m$}
	...
	\section[תת־סדרות וגבולות חלקיים]{\en{Subsequences and Partial Limits}}
	...
	\section[תוצאות שנשענות בכבדות על אקסיומת החסם העליון]{\en{Results that heavily depends on LUB}}
	...
	\theo{תהא $\an$ סדרה. אז $\an$ מתכנסת אמ''מ $\an$ סדרת קושי. }\begin{proof}\,
		\begin{itemize}
			...
			\item[$\impliedby$]נניח $\an$ סדרת קושי. קיים $N \in \N$ כך ש־$\forall n, m \ge N \co \sof{a_n - a_m} < 1$. נתבונן ב־
			\[ M = \max\{\sof{a_1}, \sof{a_2}, \dots \sof{a_{N - 1}}, \sof{a_N} + 1\} \]
			יהי $n \in \N$. אם $n \ge N$ אז $\sof{a_n} \le M$. אחרת $\sof{a_n - a_N} < 1$ ולכן $\sof{a_n} < \sof{a_N} + 1 < M$. מכאן ש־$\an$ חסומה. לפי בולצאנו־ווייראשטראס (פוף! הנחנו את אקסיומת השלמות) ל־$\anc$ יש ת''ס $a_{n_k}$ המתכנסת לגבול $\ml \in \R$.
	...
	\section[הגדרות פונקציות אלמנטריות בעזרת התכנסות]{\en{Some Elementary Functions}}
	...
	\section[טורים]{\en{Series}}
	\subsection{התכנסות טורים}
	...
	\subsubsection{מבחני השוואה לטורים אי־שליליים}
	...
	\subsubsection{מבחני השוואה לטורים משני סימן}
	...
	\begin{Theorem}[קריטריון דיריכלה]
		תהאנה $\an, \bn$ סדרות. נניח כי:
		\begin{enumerate}
	...
	\theo{ תהא $\an$ סדרה, נניח כי הטור $\sumninf$ מתכנס, אז לכל השמה של סוגריים על הסכום, הטור החדש מתכנס.} \begin{proof}
	...
	\newpage
	\npchapter{פונקציות במשתנה רציף}
	עתה נתחיל לדבר על פונקציות במשתנה רציף. קודם לכן – נעסוק קצת בטופולוגיה.
	
	\section[הטופולוגיה על הישר הממשי]{\en{The Topology on the $\R$eal Line}}
	הפרק הבא מועבר כהעשרה לאודיסאה, אך נשתמש בתוצאותיו בהמשך. הוא מגדיר מושגים מטופולוגיה בעבור הישר הממשי. חלק מההגדרות כאן נכונות עבור $\R$ בלבד, אך באופן כללי את מרבית ההגדרות כאן אפשר להכליל ל־$\R^{n}$ לכל הפחות. שימו לב, שהתכנסות תלויה אך ורק בשאלה \textit{מי הן הסביבות שלי}. התשובה לשאלה הזו, מגדירה (עד לכדי שקילות מסוימת) מושג כללי יותר שנקרא \textit{טופולוגיה}. 
	...
	\section[גבולות]{\en{Limits}}
	\subsection{מבוא – פונקציות של משתנה ממשי}
	...
	\subsubsection{קיטריוני היינה וקושי}
	...
	\begin{Theorem}[קריטריון היינה להתכנסות, או הגדרת הגבול לפי היינה]
		תהא $f \co A \subseteq \R \to \R$. תהא $x_0 \in \R$ נקודת הצטברות של $A$. ל־$f$ יש גבול ב־$x_0$ אמ''מ לכל סדרה $\an$, אם $\an$ מקיימת את 1-3 מהטענה הקודמת, $f(\an)$ מתכנסת. 
	\end{Theorem}
	...
	\begin{Theorem}[קריטריון קושי לקיום גבול של פונקציה]
		תהא \gf ותהא \lxo. ל־$f$ יש גבול סופי ב־$x_0$ אמ''מ לכל $\eg > 0$, קיים $\dg > 0$, כך שלכל $x, y \in A$ אם $0 < \sof{x - x_0} < \dg$ וגם $0 < \sof{y - x_0} < \dg$ אז $\sof{f(x) - f(y)} < \eg$. 
	\end{Theorem}
	ההוכחה זה פחות או יותר היינה עם סדרות קושי.
	...
	\subsection{רציפות כתכונה גלובלית}
	...
	\section[גזירות]{\en{Differentiability}}
	\subsection{הגדרת גזירות}
	...
	\subsubsection{נגזרות חד־צדדיות}
	...
	\subsection{אריתמטיקה של גזירות}
	...
	\subsection{גזירות כתכונה גלובאלית}
	...
	\subsubsection{המשפט היסודיים של החשבון הדיפרנציאלי}
	\rmark{יש להבדיל משפטים אלו מ''המשפט היסודי של החדו''א`` שנראה בחדו''א 2א. }
	...
	\section[פולינום טיילור]{\en{Taylor's Polynomial}}
	\subsection{הגדרת פולינום טיילור}
	...
	\subsection{משפט טיילור ותוצאותיו}
	\theo{מתקיים:
	...
	\theo{תהא $f \co I \to \R$ ותהא $x_0 \in I$ נקודת פנים. נניח $f$ גזירה $n$ פעמים ב־$x_0$. נסמן ב־$T_n$ את פולינום הטיילור של $f$ מסדר $n$ סביב $x_0$. נסמן ב־$R_n$ את השארית המתאימה. אז:
	\[ \limxz \frac{R_n(x)}{(x - x_0)^{n}} = 0 \]}
	\begin{proof}
	...
	\begin{Theorem}[שארית לגראנג']
		תהא $f \co I \to \R$ ותהא $x_0 \in \R$ בפנים הקטע. נניח כי $f$ גזירה $n + 1$ פעמים בכל $I$ ונגזרותיה רציפות (כלומר $f \in C^{(n + 1)}$). לכל $x \in I$ קיים $c$  בין $x_0$ ל־$x$ כך ש־:
	...
	יש לטורי טיילור יותר כוח ממה שאנחנו רואים כאן. אגב, בהקשר ל־$\sinx$ שטור הטיילור שלה מתכנס, זה לא נכון לכל פונקציה. לדוגמה, אם מגדיר $f \co \R\to \R$ לפי:
	\[ f(x) = \begin{cases}
		e^{-\frac{1}{x^{2}}} & x \neq 0 \\
		0 & \other
	\end{cases} \]
	אפשר להוכיח באינדוקציה של $f$, שיש לה נגזרת מסדר $n$ ב־$0$, ושלכל $n \in \N^{+}$ מתקיים $f^{(n)}(0) =0$. הוא לא מתכנס לפונקציה. 
	\rmark{זו פונקציה חשובה לסטטיסטיקה, מסתבר}
