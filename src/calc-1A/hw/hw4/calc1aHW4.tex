...
\begin{document}
	...
	\begin{enumerate}[(A)]
		...
		\[ a_n = \sumnko \frac{1}{\sqrt k} \quad b_n = {\sqrt n} \quad \implies \quad c_n = \frac{a_n}{b_n} \]
		
		ואז, נחשב את הגבול הבא, תוך היעזרות בגבול שחשבנו בהתחלה: 
		\[ \frac{a_{n + 1} - a_n}{b_{n + 1} - b_n} = \frac{\sum_{k = 1}^{n+ 1}\frac{1}{\sqrt k} - \sumnko \frac{1}{\sqrt k}}{\sqrt{n + 1} - \sqrt n} = \frac{\frac{1}{\sqrt{n + 1}}}{\sqrt{n + 1} - \sqrt n} = \frac{1}{n + 1 - (\sqrt{n^2 + n})} \toinf \frac{1}{0.5} = 2 \]
		...
		ננסה להיעזר בצ'אזרו בתקווה שהגבול קיים: 
		\[ \frac{ \sum_{i = 0}^{n + 1}\frac{(k + i)!}{i!} - \sum_{i = 0}^{n}\frac{(k + i)!}{i!}}{(n + 1)^{k + 1} - n^{k + 1}} = \frac{\frac{(k + n + 1)!}{(n + 1)!}}{(n + 1)^{k + 1} - n^{k + 1}} = \frac{(n + 1) \cdots (n + k + 1)}{\cl{(n + 1)^{k + 1} - n^{k + 1}}} = \frac{(n + 1) \cdots (n + k + 1)}{\sum_{i =0}^{k + 1}\cl{\binom{k + 1}{i}n^{i}} - \sum_{i =0}^{k}\cl{\binom{k}{i}n^{i}}} =: \frac{P}{Q} \]
		
		אחרי שפתחנו את הבינום של ניוטון להנאתנו, נחלק ב־$n^{k}$ את שני האגפים. לכל $n \ge 3k$ מתקיים: 
		\[ \frac{P}{n^{k}} \ge \frac{n + 1}{n^2} \cdot \frac{n + 2}{n^2} + \cdots \frac{n + k + 1}{n^2} \ge \frac{n^2}{n^2} = 1 \]
		מזהות פסקל $\binom{n + 1}{i} - \binom{n}{i} = \binom{n - 1}{i - 1}$. גם כאן, לכל $n \ge 2k$: 
		\[ \frac{Q}{n^{k}} = \frac{(k + 1)n^{k} + \sum_{i = 1}^{n}\cl{\binom{n + 1}{i} - \binom{n}{i}}n^{i}}{n^{k}} = (k + 1) + \sumni \underbrace{\frac{(i - 1) \cdots (n - 1) \cdot n^{i}}{n^{k}}}_{ \ge \frac{n^{2k - 2}}{n^{3k}}} \ge (k + 1) + \sumni \frac{1}{i^{n}} \toinf (k + 1) \]
		סה''כ קיבלנו מאריתמטיקת גבולות: 
		\[ \trio = \frac{P}{Q} = \frac{\frac{P}{n^{k}}}{\frac{Q}{n^{k}}} \toinf \frac{1}{k + 1} \]
		כלומר הגבול קיים, ולכן ממשפט צ'אזרו הגבול הכיפי למעלה שואף ל־$\frac{1}{k + 1}$ וסיימנו. 		
	\end{enumerate}
	...
	\begin{proof}
		...
		נבחין ש־$a_n$ מונוטונית יורדת חסומה ע''י $0$ ולכן מתכנסת. נבחין ש־$b_n$ מונוטונית עולה חסומה ע''י $a_1$ ולכן מתכנסת. סה''כ שתיהן מתכנסות, וממשפט בגלל ש־$a_n > b_n$, לאותו הגבול. (TODO: להראות ש־$\limsi a_n - b_n = 0$)
	...
	\begin{proof}
		קל לראות ש־$0 \le a_n \le 1$ (נטו מתחומי ההגדרה של הפונקציות). בשיעורי בית קודמים הוכחנו ש־$\ccb{\sqrt x}$ בעלת איפימום $0$, וסופרמום $1$, ומכאן (עם מעט מאוד מניפולציות אלגבריות) ש־$\sup \an = 1 \land \inf \an = 0$. ידוע ש־: 
		\[ \sqrt{n} - {\sqrt {n + 1}} \cdot \frac{\sqrt{n} + \sqrt{n + 1}}{\sqrt{n} + \sqrt{n + 1}} = \frac{1}{\sqrt n + \sqrt{n + 1}} \to 0 \]
		נבחין מתנהגות המודלו: 
		\[ 0 = -0 \leftarrow -\sqrt{n} + \sqrt{n + 1} \le a_n -a_{n + 1}\le \sqrt{n} - \sqrt{n + 1} \to 0 \]
		ש־$a_n - a_{n + 1} \to 0$. סה''כ ממשפט 6 בהכרח $\ps(a_n) = [0, 1]$ כנדרש. 
	\end{proof}
	...
	\begin{proof}
		נתחיל מהמקרה הרציונלי. יהי $q \in \Q\cap [0, 1]$, אז קיימים $n, m \in \N$ כך ש־$q = \frac{n}{m}$ וגם $m > n$. נגדיר את הסדרה: 
		\[ n_k = (mk)^{2} + 2nk \]
		נבחין ש־: 
		\[ (bk)^2 < \underbrace{(mk)^{2} + 2nk}_{n_j} < (mk)^22  + 2bk\cdot 1 + 1 = (mk + 1)^{2} \implies \floor{\sqrt{n_k}} = bk \]
		נותר להראות ש־$a_{n_j}$ הינה ת''ס שואפת ל־$q$. אכן: 
		\begin{alignat*}{9}
			a_{n_j} &= \sqrt{{n_j}} - \floor{\sqrt{{n_k}}} &&= \sqrt{{n_k}} - \floor{\sqrt{{n_k}}} \cdot \frac{\sqrt{{n_k}} + \floor{\sqrt{{n_k}}}}{\sqrt{{n_k}} + \floor{\sqrt{{n_k}}}} \\
			&= \frac{n_k - \floor{\sqrt{n_k}}^{2}}{\sqrt{n_k} + \floor{\sqrt{n_k}}} &&= 
			\frac{\cancel{(mk)^{2} - (mk)^{2}} + 2nk}{\sqrt{(mk)^2 + 2nk} + mk} \\
			&= \frac{\frac{2n\cancel{k}}{\cancel k}}{\sqrt{\frac{m^2\cancel{k^2}}{\cancel{k^2}} + 2\frac{n\cancel{k}}{k^{\cancel{2}}}} + \frac{m\cancel{k}}{\cancel{k}}} &&= \frac{2n}{\sqrt{m^2 + \frac{m}{k}} + m} \rrr{k \to \infty} \frac{2n}{\sqrt{m^2} + m} = \frac{\cancel{2}n}{\cancel{2}m} = \frac{n}{m} = q
		\end{alignat*}
		דהיינו $q$ גבול חלקי של $a_n$. 
		
		\textit{הערה: לא אני מצאתי את הת''ס הזו. הקרדיט שייך לנגה מפיזיקה. }
		
		עתה, יהי $r \in \R \cap [0, 1]$ מספר כלשהו. נראה שהוא גבול חלקי לפי אחת מההגדרות השקולות. נסמן $A = \Img \an$ לנוחות. יהי $\eg > 0$. נתבונן בסביבה $U=(r -\eg, r + \eg)$. מצפיפות הרציונליים בממשיים קיימים $q_1, q_2 \in U \cap \Q$ כך ש־$q_1 < r < q_2$. בגלל שהם גבול חלקי של $\az$, בכל סביבה נקובה סביבם יש אינסוף איברים מ־$A$, ובפרט בעבור $\eg_2 := \frac{r + \eg + q_2}{2}$ ו־$\eg_1 = \frac{r+ \eg + q_1}{2}$ והסביבות שהן משרים המקיימות: 
		\[ q_1 \in \underbrace{(r - \eg_1, r + \eg_1)}_{U_1} \subseteq (r - \eg, r + \eg) \supseteq \underbrace{(r - \eg_2, r + \eg_2)}_{U_2} \ni q_2 \]
		יש אינסוף איברים מ־$A$, כלומר $\sof{U_1 \cap A}, \sof{U_2 \cap A} \ge \az$, ומכאן שבהכרח $\sof{A \cap U} \ge \az$ (ממשפטי עוצמות) ומהגדרה שקולה שהראינו מתישהו, $r$ גבול חלקי. 
		
		(השתמשו בעובדה ש־$r \in \R\cap [0, 1]$ ולא $r \in \R$ כללי, כאשר טענו שהסביבות $U_1, U_2$ מוכלות ב־$U$)
	\end{proof}
	...
	\begin{proof}
		יהי $(\R, +, \cdot, <)$ שדה סדור מלא. בכל קונטקסט בו המושגים מוגדרים (דהיינו קשורים לכמתים), נסמן $\pi_1(c_n) =: b_0$ ו־$\pi_2(c_n) =: a_0$. 
		
		תהי $\an$ סדרה חסומה. בפרט $-M < a_n < M$ עבור $M$ כלשהו. אם $\Img \an$ סופי, אז ת''ס קבועה קיימת בה בהכרח וסיימנו. אחרת, נגדיר את הסדרה הבאה: 
		...
		\end{itemize}
		עתה נגדיר את הסדרות: 
		\[ b_n \co \N \to \R \quad b_n = \pi_1 \circ c_n \quad a_n \co \N \to \R \quad a_n = \pi \circ c_n \]
		מהגדרת $c_n$ מתקיים ש־$a_n \le a_{n + 1} \le b_{ n+ 1} \le b_n$. יהי $\eg > 0$. ידוע קיום $N \in \N$ עבורו $\frac{M}{2^{n - 1}} < \eg$ (כן. צריך בשביל זה ארכימדיאניות. בדקתי, גם בויקיפדיה מופיעה אותה ההוכחה. גם בהרצאה אמרתם לנו להוכיח עם אריה במדבר. זה לא עובד, צריך ארכימדיאניות. למעשה כמו שהראה בהמשך התרגיל, מספיק קיום של סדרה מונוטונית עולה חיובית, אבל זה סיוט לאחר־כך, ובכל מקרה זה לא עובד בשדה סדור מלא כללי). בגלל שכל פעם אנו חוצים את הקטעים פי $2$, נקבל ש־$b_n - a_n = \frac{M}{2^{n - 1}}$. מכאן ש־: 
